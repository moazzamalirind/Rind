...
*****Description: This model is developed to find a cooridated optimal reservoir operation of Lake Powell and Lake mead to benefit both water supply and Ecosystem objectives.
...
Set
             ...
             S                  Saving results from each of the model. There are total 2 models to find the extreme points and 2 additional scenarios /Max,Min,Valid,Frac/
;
...
Init_Storage(Loc)                     Initial reservoir storage i.e. storage measured on 30th september as per calender year and one day before the start of water year calender(acre-ft)/Powell 14664000,Mead  10182000/
...
*Here we are making 29 days in feburary because of leap year.
Yr_Month_Day("y3","M5")= 29;

*===================================================
* Read data from Excel
*===================================================
$CALL GDXXRW.EXE input=Input_2018-20WY.xlsx output=3yr_Results.gdx  par=Inflow_Powell rng=Inflow_powell!A1 Rdim=2  par=Inflow_Paria rng=Inflow_paria!A1 Rdim=2 par=Inflow_Local rng=Inflow_Local!A1 Rdim=2  par=Rel_Powell rng=Rel_Powell!A1 Rdim=2  par=Rel_Mead rng=Rel_Mead!A1 Rdim=2

*Write the input Data into a GDX file
$GDXIN 3yr_Results.gdx
...
*Constraints
EQ3_EndofTime_Storage(Loc)..                                    Storage(Loc,"y3","M12")=g= Init_Storage(Loc);
...
*********************************************************************************************
Parameters
Monthly_Frac(M)                      Volume fraction contributed by each of the months towards annual release volume/M1 0.3213,M2 0.3208,M3 0.3579,M4 0.1225,M5 0.1083,M6 0.1144,M7 0.1020,M8 0.1015,M9 0.1065,M10 0.1202,M11 0.1284,M12 .0963/;
*for the glen canyon dam 2 MAF release volume is distributed amongst first three months and remaining volume distributed amongst the other months in the ratio described in Monthly_ Fraction.


*===========================================================================================
* The  Scenario presented in the model was motivated from the monthly volume distribution given in the Glen Canyon Dam monthly model given by WAPA.
*In case of Lake mead we are considering 3 MAF release in first 3 months of the water year and for the remaining months the proportion of release in each of the months was consider similar to Glen Canyon Dam.

EQUATION
*Monthly release constraints for Glen Canyon Dam
EQ15_1stThreeMonths_GCD(Loc,yr,M)           Total volume distribution during  first three months of the water year (October Novermber and December) from GCD (ac-ft)
EQ16_SevenMonths_FracGCD(Loc,yr,M)          Release volume distribution during remaining 7 months of the water year from GCD (ac-ft)

*Monthly release constraints for Hoover Dam
EQ17_1stThreeMonths_Hoover(Loc,yr,M)        Total volume distribution during  first three months of the water year (October Novermber and December) from Hoover Dam (ac-ft)
EQ18_SevenMonths_FracHoover(Loc,yr,M)       Release volume distribution during remaining 7 months of the water year from Hoover Dam (ac-ft)
EQ19_RelVol_Constriant(Loc,yr)              Assuming no reservior has annual release volume of than 12 MAF (ac-ft);

*===========================================================================================

EQ15_1stThreeMonths_GCD(Loc,yr,M)$(ord(Loc) eq 1 and ord(M) lt 4)..             Release(Loc,yr,M)*Convert*Yr_Month_Day(yr,M) =e= 2000000*Monthly_Frac(M) ;
*2 MAF release in first 3 months of the Water year.

EQ16_SevenMonths_FracGCD(Loc,yr,M)$(ord(Loc) eq 1 and ord(M) gt 3)..           Release(Loc,yr,M)*Convert*Yr_Month_Day(yr,M) =e= (WY_Volume(Loc,yr)-2000000)*Monthly_Frac(M);

EQ17_1stThreeMonths_Hoover(Loc,yr,M)$(ord(Loc) eq 2 and ord(M) lt 4)..          Release(Loc,yr,M)*Convert*Yr_Month_Day(yr,M) =e= 3000000*Monthly_Frac(M) ;
*3 MAF release in first 3 months of the Water year.

EQ18_SevenMonths_FracHoover(Loc,yr,M)$(ord(Loc) eq 2 and ord(M) gt 3)..        Release(Loc,yr,M)*Convert*Yr_Month_Day(yr,M) =e= (WY_Volume(Loc,yr)-3000000)*Monthly_Frac(M);

EQ19_RelVol_Constriant(Loc,yr)..                                               WY_Volume(Loc,yr)=L= 12000000;
*===========================================================================================

*,EQ3_EndofTime_Storage
Model Model4 validation model with made up release values/EQ1_Massblc_Powell,EQ2_Massblc_Mead,EQ4_MaxStorage,EQ5_MinStorage,EQ6_MaxRel,EQ7_MinRel,EQ8_AnnualVol,EQ9_obj_temp,EQ10_UB_Delivery,EQ11_obj_Mead,EQ12_obj_MeadLB,EQ13_CombinedObjectives,EQ15_1stThreeMonths_GCD,EQ16_SevenMonths_FracGCD,EQ17_1stThreeMonths_Hoover,EQ18_SevenMonths_FracHoover,EQ19_RelVol_Constriant/;
...
*Here we are miximizing all the objectives, therefore, set obj_dir equals 1
 Obj_dir=1;
SOLVE Model4 using NLP Maximize CombineObjective;

Objective_Val(Objective,"Frac")= CombineObjective.L

loop((Location,Y,Month),
*saving values
Rstore(Objective,Location,"Frac",Y,Month)= Release.L(Location,Y,Month);
Sstore(Objective,Location,"Frac",Y,Month)= Storage.L(Location,Y,Month);
Rel_vol(Objective,Location,"Frac",Y)= WY_Volume.L(Location,Y);

ModelResults(Objective,"Frac",Y,Month,"SolStat")=Model4.solvestat;
ModelResults(Objective,"Frac",Y,Month,"ModStat")=Model4.modelstat;
);
...
Execute_Unload "3yr_Results.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls 3yr_Results.gdx"
